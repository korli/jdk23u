...
// The attach mechanism on Haiku uses a UNIX domain socket. An attach listener
...
// forward reference
class HaikuAttachOperation;

class HaikuAttachListener: AllStatic {
 private:
 ...
 public:
  ...
  static HaikuAttachOperation* dequeue();
};

class HaikuAttachOperation: public AttachOperation {
 private:
 ...
 public:
  ...
  HaikuAttachOperation(char* name) : AttachOperation(name) {
    set_socket(-1);
  }
};
...
HaikuAttachOperation* HaikuAttachListener::read_request(int s) {
  ...
  HaikuAttachOperation* op = new HaikuAttachOperation(name);
...
//
// In the Haiku implementation there is only a single operation and clients
// cannot queue commands (except at the socket level).
//
HaikuAttachOperation* HaikuAttachListener::dequeue() {
  for (;;) {
    int s;
...
int HaikuAttachListener::write_fully(int s, char* buf, int len) {
  do {
...
void HaikuAttachOperation::complete(jint result, bufferedStream* st) {
  ...
  if (rc == 0) {
    HaikuAttachListener::write_fully(this->socket(), (char*) st->base(), st->size());
...
AttachOperation* AttachListener::dequeue() {
  ...
  AttachOperation* op = HaikuAttachListener::dequeue();
...
int AttachListener::pd_init() {
  ...
  int ret_code = HaikuAttachListener::init();
